# Bedrock Guardrails ドキュメント

AWS Bedrock Guardrails の設計・実装・運用に関するドキュメント集です。

## 📚 ドキュメント一覧

### [設計ガイド](./design-guide.md) 🆕

Bedrock Guardrails を新規に設計する際のワークフローとチェックリスト。**仕事で使える実践的なドキュメントです。**

**主な内容:**
- ✅ 設計フロー概要（6つのフェーズ）
- ✅ 要件定義（ビジネス・セキュリティ・技術）
- ✅ ポリシー設計（コンテンツフィルター、トピック、PII、ワード）
- ✅ 実装・設定（Terraform例、環境別設定）
- ✅ テスト・検証（テストケース、自動化、負荷テスト）
- ✅ 本番デプロイ（段階的ロールアウト、ロールバック）
- ✅ 運用・改善（モニタリング、定期レビュー）
- ✅ **総合チェックリスト**（全フェーズ網羅）
- ✅ **ユースケース別推奨設定マトリクス**
